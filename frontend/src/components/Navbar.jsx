import React from 'react';
import { Link, useLocation, useNavigate } from 'react-router';

import useAuthentication from '../Auth';
import favicon from '../assets/images/VV-logo-favicon.png';
import faviconblue from '../assets/images/VV-logo-blue-large.png';
import { useTheme } from '../ThemeContext';

export default function Navbar() {
  // The navbar is mounted globally, so it owns the theme toggle for the whole app.
  const { theme, setTheme } = useTheme();
  const { isAuthorized, logout } = useAuthentication();
  const location = useLocation();
  const navigate = useNavigate();

  const toggleTheme = () => {
    setTheme(theme === 'nord' ? 'halloween' : 'nord');
  };

  const handleLogout = () => {
    logout();
    navigate('/');
  };

  return (
    <div className="navbar bg-base-200 shadow-sm border-3 border-base-200 border-b-primary">
      <div className="navbar-start">
        <div className="dropdown">
          <div tabIndex={0} role="button" aria-label="dropdown button" className="btn btn-ghost btn-circle">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5 text-primary"
              fill=""
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M4 6h16M4 12h16M4 18h7"
              />
            </svg>
          </div>
          <ul
            tabIndex="-1"
            className="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow"
          >
            <nav>
              {isAuthorized ? (
                <li className="bg-base-100 focus:border-2-primary text-primary">
                  <Link aria-label="Logout" onClick={handleLogout} to="/">
                    Logout
                  </Link>
                </li>
              ) : location.pathname === '/login' ? (
                <>
                  <li className="bg-base-100 focus:border-2-primary text-primary">
                    <Link aria-label="Home" to="/">Home</Link>
                  </li>
                  <li className="bg-base-100 focus:border-2-primary text-primary">
                    <Link aria-label="Register" to="/register">Register</Link>
                  </li>
                </>
              ) : (
                <>
                  <li className="bg-base-100 focus:border-2-primary text-primary">
                    <Link aria-label="Home" to="/">Home</Link>
                  </li>
                  <li className="bg-base-100 focus:border-2-primary text-primary">
                    <Link aria-label="Login" to="/login">Login</Link>
                  </li>
                  <li className="bg-base-100 focus:border-2-primary text-primary">
                    <Link aria-label="Register" to="/register">Register</Link>
                  </li>
                </>
              )}

              <li>
                <label className="swap swap-rotate bg-base-100 focus:border-2-primary text-primary">
                  <input
                    type="checkbox"
                    className="theme-controller"
                    value="halloween"
                    checked={theme === 'halloween'}
                    onChange={toggleTheme}
                  />

                  <svg
                    className="swap-off h-5 w-5 fill-current font-accent"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                  >
                    <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
                  </svg>

                  <svg
                    className="swap-on h-5 w-5 fill-current font-accent"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                  >
                    <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
                  </svg>
                </label>
              </li>
            </nav>
          </ul>
        </div>
      </div>
      <div className="navbar-end">
        <div className="w-16 m-4">
          <img src={theme === 'halloween' ? favicon : faviconblue} alt="vitality-vault logo" />
        </div>
      </div>
    </div>
  );
}
