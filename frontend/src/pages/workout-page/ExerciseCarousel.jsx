...
{exerciseImages.map((exercise, index) => {
 ...
 const previous = index === 0
  ...
  return (
 <div id={`slide${index + 1}`} className="carousel-item relative w-full justify-center items-center justify-self-center align-self-center bg-primary border-y-8 border-accent  mb-4 gap-4" key = {exercise.id}>
   <img
    ...
    <div className="absolute left-10 right-10 top-1/2 flex -translate-y-1/2 transform justify-between">
      <a href={`#slide${previous}`} className="btn btn-circle bg-accent">❮</a>
      <a href={`#slide${next}`} className="btn btn-circle bg-accent">❯</a>
